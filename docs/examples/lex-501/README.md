...
recipe; the command below reproduces the result, and editing either file is how the result
is improved.
...
- `key_terms`: names Deepgram must get right. Without them the transcript spelled Omarchy
  three different ways about eighty times. Changing them re-runs speech-to-text (about
  twelve minutes plus the request) and everything after it.
...
- `segments.yml`: stretches to mark and keep out of the analysis. Editing an existing hint
  resumes at the outline, about twenty minutes here; the first hints file on a source
  repeats paragraph formatting and section headings once.
